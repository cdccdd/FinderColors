...
def ColorsName(peaked_color):
    ...
    # Получить список значений цвета в формате шестнадцатеричной строки
    ...
    rgb = np.dstack(
        (r, g, b)
    )  # Stack r,g,b across third dimention - create to 3D array (of R,G,B elements).
    ...
    peaked_rgb = np.asarray(
        [
            int(peaked_color[1:3], 16),
            int(peaked_color[3:5], 16),
            int(peaked_color[5:7], 16),
        ],
        np.uint8,
    )
    ...
    lab_dist = (
        (lab[:, :, 0] - peaked_lab[:, :, 0]) ** 2
        + (lab[:, :, 1] - peaked_lab[:, :, 1]) ** 2
...
# print("Peaked color name: " + ColorsName(i))
