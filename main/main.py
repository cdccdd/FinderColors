...
current_dir = os.getcwd()  # текущая директория E:\Maxim\Github\FinderColor\main
image_dir = current_dir.rstrip('\main') + '\Image\\' # путь по директории с картинками
text_dir = current_dir.rstrip('\main') + '\Text_files\\' # путь по директории с текстовыми файлами
...
frame_title = tk.Frame(window, width=1224, height=50, bg='#f0f0f0')  # , bg='green'f0f0f0
frame_title.place(relwidth=1, height=90)

frame_methods = tk.Frame(window, width=1224, height=100, bg='#f0f0f0')  # , bg='blue'
frame_methods.place(y=90, relwidth=1, height=350)

frame_result = tk.Frame(window, width=1224, height=100, bg='#f0f0f0')  # , bg='red'
...
frameCnt = 60
title_gif = [tk.PhotoImage(file=image_dir+'title.gif', format = 'gif -index %i' %(i)) for i in range(frameCnt)]
...
def update(ind):
    ...
    frame_title.after(100, update, ind)
l_title_name = tk.Label(frame_title)
l_title_name.pack()
frame_title.after(0, update, 0)
...
image_opposite = ImageTk.PhotoImage(Image.open(image_dir+"opposite.jpeg").resize( (120, 130) ))
...
image_classic_thiada = ImageTk.PhotoImage(Image.open(image_dir+"classic_thiada.jpeg").resize( (120, 130) ))
...
image_analog_thriada = ImageTk.PhotoImage(Image.open(image_dir+"analog_thriada.jpeg").resize( (120, 130) ))
...
image_contrast_thriada = ImageTk.PhotoImage(Image.open(image_dir+"contrast_thriada.jpeg").resize( (120, 130) ))
...
image_rectangle = ImageTk.PhotoImage(Image.open(image_dir+"rectangle.jpeg").resize( (120, 130) ))
...
image_square = ImageTk.PhotoImage(Image.open(image_dir+"square.jpeg").resize( (120, 130) ))
...
def window_wheel():
    ...
    l_image_ryb = tk.Label(windowwel, image=load_image_ryb).pack()

    exit_button = tk.Button(windowwel, text="Закрыть", command=lambda: windowwel.destroy()).pack()
    windowwel.mainloop()
